...
Func RunExchange()
	...
	If $connRetail.КонфигурацияИзменена() Then
		...
		If RunUpdateCfg() Then
			; Отключил автоматический запуск клиентского приложения
			; под полными правами. Полностью эта функция не проверена
			; При необходимости можно запустить с параметром ком.строки
			;$bApplyCfg	= True
		Else
